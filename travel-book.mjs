...
export function cleanBookPages(value){
  ...
  return value.slice(0,12).map(p=>({title:String(p?.title||'').slice(0,80),text:String(p?.text||'').slice(0,1000),caption:String(p?.caption||'').slice(0,160),image:typeof p?.image==='string'&&/^photo-[a-zA-Z0-9-]{1,80}$/.test(p.image)?p.image:''}));
}
...
export function createTravelBook({width,height,paintSpread,spreadCount,anisotropy=4,document:doc}){
  ...
  for(const sign of [-1,1]){
    ...
    const cover=new THREE.Mesh(coverGeo,leather);cover.position.set(sign*width/4,.025,0);book.add(cover);
    addBox(width/2-.015,.11,height,sign*width/4,.14,0,paper);
    ...
    book.add(new THREE.LineSegments(new THREE.BufferGeometry().setFromPoints(lines),edgeMat));
  }
  ...
  const left=makeStatic(-1),right=makeStatic(1);staticGeo.dispose();
  ...
  book.userData={width,height,spreadCount,index:0,turn:0,manualTurn:null,surface:left,setState,dispose:()=>{for(const pair of cache.values())pair.forEach(t=>t.dispose());cache.clear();}};
