...
writeBlock_fn = function(chunk, canCreateNewCluster) {
  ...
  // MountainAnimator: include the final video frame in the file duration.
  const frameRate = __privateGet(this, _options).video?.frameRate;
  const endTimestamp = chunk.trackNumber === VIDEO_TRACK_NUMBER && frameRate
    ? Math.round(chunk.timestamp / 1e3 + 1000 / frameRate) : msTimestamp;
  __privateSet(this, _duration, Math.max(__privateGet(this, _duration), endTimestamp));
};
