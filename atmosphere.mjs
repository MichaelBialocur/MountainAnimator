...
export const CLOUD_TYPES=['cumulus','stratus','cirrus','cumulonimbus'];
export function cloudVolume(type,detail,seed,size){
  const scale=type==='cumulonimbus'?[2.8,3.8,2.2]:type==='stratus'?[2.8,.36,1.6]:type==='cirrus'?[3.2,.2,1.1]:[1.5,.95,1.2];
  const material=new THREE.ShaderMaterial({transparent:true,depthTest:true,depthWrite:false,side:THREE.BackSide,
    uniforms:{storm:{value:type==='cumulonimbus'?1:0},eye:{value:new THREE.Vector3()},lightDir:{value:new THREE.Vector3(.5,1,.4)},opacity:{value:.7},seed:{value:seed%1000},steps:{value:[0,24,40,64][detail]||40}},
    ...
    varying vec3 localPos;uniform vec3 eye,lightDir;uniform float opacity,seed,storm;uniform int steps;
    ...
    float density(vec3 p){float edge=1.-smoothstep(.27,.5,length(p));
    if(storm>.5){float tower=1.-smoothstep(.65,1.,length((p-vec3(0.,-.12,0.))/vec3(.24,.38,.28)));float anvil=1.-smoothstep(.65,1.,length((p-vec3(.02,.28,0.))/vec3(.48,.14,.42)));edge=max(tower,anvil);}float n=.57*noise(p*7.)+.28*noise(p*16.)+.15*noise(p*35.);return edge*smoothstep(.27,.65,n)*5.;}
    ...
    for(int i=0;i<64;i++){if(i>=steps||sum.a>.98)break;vec3 p=eye+rd*(t+(float(i)+.5)*dt);float d=density(p);float shade=exp(-density(p+light*.12)*.7-density(p+light*.25)*.3);vec3 color=mix(mix(vec3(.39,.47,.56),vec3(.19,.24,.30),storm),vec3(1.,.97,.91),shade);float alpha=1.-exp(-d*dt*opacity*5.);sum.rgb+=(1.-sum.a)*alpha*color;sum.a+=(1.-sum.a)*alpha;}
